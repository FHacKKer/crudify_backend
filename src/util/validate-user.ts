 ...
 */
export function validateUser(inputUser: IUser): IUserValidationResult {
  const { name, username, email, password, role } = inputUser;

  /**
   * safe user removed all additional properties
   */
  const user = {
    name,
    username,
